...
__all__ = ['WorkLoanQuerySet', 'TypeWorkLoanQuerySet']

if TYPE_CHECKING:
    from .models import WorkLoan

    class _Base(QuerySet[WorkLoan]):
        pass
else:
...
class WorkLoanQuerySet(_Base):
    class Filters:
        open = Q(status=WorkLoanStatus.Open, closed_at__isnull=True)
        closed = Q(status=WorkLoanStatus.Closed, closed_at__isnull=False)
...
TypeWorkLoanQuerySet = TypeVar('TypeWorkLoanQuerySet', bound=WorkLoanQuerySet)
